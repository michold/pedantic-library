...
class MoveFolders
  ...
  def update!
    return abort("files are sorted properly".green) if same_path? && files.map(&:to_ascii) == files
  ...
  def move_files_to_new_folder
    files.each do |file_path|
      ...
      if new_file_name != file_name
        next unless approved_by_prompt("rename file from `#{file_name}` to `#{new_file_name}`")
      end
